...
3. **Crowd heat meter sits at the bottom all match** (user: "makes the matches seem pretty boring"). Heat decays a flat 3/s while bumps are small one-shots, so it bleeds to zero between spots. Ideas to explore: slower/asymmetric decay (fast above 70, slow below), a floor that ratchets up with match milestones (nearfalls, comebacks), bigger bumps for chained spots, and/or making the meter show a rolling average so it reads as sustained excitement rather than instantaneous. Note heat has gameplay teeth now (comeback scaling, taunt conversion) — retune those if the curve changes.
